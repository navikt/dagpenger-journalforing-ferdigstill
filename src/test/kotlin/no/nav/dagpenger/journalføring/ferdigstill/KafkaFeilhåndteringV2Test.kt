...
class KafkaFeilhåndteringV2Test {
    private val configuration = Configuration()
    private val dagpengerJournalpostTopicV2 = configuration.kafka.dagpengerJournalpostTopicV2
    ...
    val factory = ConsumerRecordFactory<String, Packet>(
        dagpengerJournalpostTopicV2.name,
        dagpengerJournalpostTopicV2.keySerde.serializer(),
        dagpengerJournalpostTopicV2.valueSerde.serializer()
    )
    ...
    fun `skal fortsette der den slapp når noe feiler`() {
        ...
        val application = ApplicationV2(configuration, journalFøringFerdigstill, FakeUnleash())
    ...
    fun `skal feile hvis pakken er lest mer enn 10 ganger`() {
        val application = ApplicationV2(configuration, mockk(), FakeUnleash())
    ...
    private fun readOutput(topologyTestDriver: TopologyTestDriver): ProducerRecord<String, Packet>? {
        return topologyTestDriver.readOutput(
            dagpengerJournalpostTopicV2.name,
            dagpengerJournalpostTopicV2.keySerde.deserializer(),
            dagpengerJournalpostTopicV2.valueSerde.deserializer()
        )
    }
}
