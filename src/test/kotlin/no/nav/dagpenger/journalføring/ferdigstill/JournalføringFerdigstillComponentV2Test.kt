...
internal class JournalforingFerdigstillComponentV2Test {
    ...
    companion object {
        ...
        val embeddedEnvironment = KafkaEnvironment(
            ...
            withSecurity = true,
            topicInfos = listOf(KafkaEnvironment.TopicInfo("privat-dagpenger-journalpost-mottatt-v2"))
        )
        ...
        private val app = ApplicationV2(configuration, journalFøringFerdigstill, FakeUnleash())
        @BeforeAll
        @JvmStatic
        fun setup() {
    ...
    fun ` Component test of JournalføringFerdigstill`() {
        ...
        behovProducer(configuration).run {
            this.send(ProducerRecord(configuration.kafka.dagpengerJournalpostTopicV2.name, packet)).get()
        }
    ...
    private fun behovProducer(configuration: Configuration): KafkaProducer<String, Packet> {
        val topic = configuration.kafka.dagpengerJournalpostTopicV2
